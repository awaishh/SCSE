    ...
    return (
      <div className="min-h-screen bg-[#13121B] font-['Satoshi'] flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-sm text-[#A9A8B8] mb-6">{error || "This replay hasn't been finalized yet."}</p>
          <button
            onClick={() => navigate("/dashboard")}
            className="px-5 py-2.5 bg-[#B7FF2A] text-[#13121B] text-sm font-semibold rounded-lg"
          >
            Back to Dashboard
          </button>
        </div>
    ...
    <div className="min-h-screen bg-[#13121B] text-white font-['Satoshi']">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        <div className="mb-8">
          ...
          <p className="text-[#A9A8B8] text-sm mt-1">Review what each player submitted</p>
        </div>

        {/* Final scoreboard */}
        {replay.finalScoreboard?.length > 0 && (
          <div className="mb-8 border border-[#302E46] rounded-xl overflow-hidden shadow-sm">
            <div className="px-5 py-3 bg-[#1C1A2A] border-b border-[#302E46]">
              <p className="text-xs font-semibold text-[#A9A8B8] uppercase tracking-wide">Final Result</p>
            </div>
            {replay.finalScoreboard.map((p, i) => (
              <div key={i} className={`flex items-center justify-between px-5 py-4 ${i > 0 ? "border-t border-[#302E46]" : ""}`}>
                ...
                <div className="flex items-center gap-4 text-xs text-[#A9A8B8]">
                  ...
                  <span className="font-black text-[#B7FF2A] text-sm">{p.score || 0} pts</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {playersData.length === 0 ? (
          <div className="text-center py-16 border-2 border-dashed border-[#302E46] rounded-xl">
            <p className="text-[#A9A8B8] text-sm">No submissions were recorded for this match.</p>
          </div>
        ) : (
                  ...
                  {player.subs.map((sub, i) => (
                    <div key={i} className="flex flex-col group hover:bg-[#1C1A2A] transition-colors">
                      <div className="w-full flex items-center justify-between px-5 py-3.5 text-left">
                        <div>
                          <p className="text-sm font-semibold text-white">{sub.problemTitle}</p>
                          <p className="text-xs text-[#A9A8B8] mt-0.5">{sub.language} · {sub.history.length} snapshots</p>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="bg-[#B7FF2A] text-[#13121B] text-[10px] font-bold px-3 py-1.5 rounded-lg hover:bg-[#1C1A2A] transition-all opacity-0 group-hover:opacity-100"
                          >
                            View
                          </button>
                        </div>
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-[#181827] rounded-3xl w-full max-w-3xl shadow-2xl overflow-hidden border border-[#302E46]">
            {/* Modal header */}
            <div className="flex items-center justify-between px-8 py-6 border-b border-[#302E46] bg-[#1C1A2A]/50">
              <div>
                <p className="font-bold text-lg text-white">{selected.problemTitle}</p>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-xs text-[#A9A8B8]">{selected.playerName} · {selected.language}</span>
                  <span className="text-gray-300">·</span>
                  <span className={`text-xs font-bold ${selected.verdict === "Accepted" ? "text-[#B7FF2A]" : "text-red-500"}`}>
                    {selected.verdict}
                  </span>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-[#1C1A2A] text-[#A9A8B8] hover:text-[#A9A8B8] transition-all text-xl"
                >
                  ✕
                </button>
              </div>
